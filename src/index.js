...
export default class SvgAnimatedLinearGradient extends Component {
  constructor(props) {
    ...
    this.AnimatedSvg = Animated.createAnimatedComponent(props.svgComponent);
  }
  offsetValueBound(x) {
    if (x > 1) {
      return '1';
  ...
  render() {
    const AnimatedSvg = this.AnimatedSvg;
    const Svg = this.props.svgComponent;
    return (
      <AnimatedSvg height={this.props.height} width={this.props.width}>
        <Svg.Defs>
          ...
          <Svg.ClipPath id="clip">
            <Svg.G>{this.props.children}</Svg.G>
          </Svg.ClipPath>
        </Svg.Defs>
...
SvgAnimatedLinearGradient.propTypes = {
  ...
  x2: PropTypes.string,
  y2: PropTypes.string,
  svgComponent: PropTypes.function.require
};
